...
func main() {
	...
	db, err = sql.Open("mysql", "root:root@tcp(127.0.0.1:3306)/studentdb")
	if err != nil {
		panic(err.Error())
	}
	defer db.Close()
	...
	router.HandleFunc("/getstudent", controller.getStudent).Methods("GET")
	router.HandleFunc("/getstudent", student_controller.createstudent).Methods("POST")
	router.HandleFunc("/getstudent/{id}", student_controller.getStudentById).Methods("GET")
	router.HandleFunc("/getstudent/{id}", student_controller.deleteStudent).Methods("DELETE")
